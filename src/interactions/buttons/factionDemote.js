...
module.exports = {
  match: (customId) => startsWith(customId, 'faction', 'demote'),
  async execute(interaction, parts) {
    ...
    const result = await factionService.changeRank(interaction.guild, factionId, targetId, 'down');
    if (!result.ok) {
      const reason = result.reason === 'no_further_rank' ? 'Ten członek ma już najniższą rangę.' : 'Nie jest członkiem tej frakcji.';
      await interaction.reply({ embeds: [errorEmbed(reason)], ephemeral: true });
      return;
    }

    await interaction.reply({
      embeds: [successEmbed(`Zdegradowano <@${targetId}> do rangi **${result.newRank.name}**.`)],
    });
  },
};
